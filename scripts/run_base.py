...
if __name__ == '__main__':
    ...
    for point_cloud_filename in point_clouds_to_process:
        parameters = dict(point_cloud_filename=point_cloud_filename,
                          ...
                          plot_radius = 13.84,    # If 0 m, the plot is not cropped. Otherwise, the plot is cylindrically cropped from the plot centre with plot_radius + plot_radius_buffer.
                          plot_radius_buffer=3.16, # See README. If non-zero, this is used for "Tree Aware Plot Cropping Mode". It was 1.9m but it's too small for leaning trees outside. 

                          # DBH height - height above ground to take DBH measurement
                          dbh_height = 1.3,      # 1.4m for New Zealand, 1.3m for the rest of the world
                          minimum_DBH = 0.045,  # (metres) trees having DBH smaller than minimum_DBH will be deleted
                          maximum_DBH = .6,
